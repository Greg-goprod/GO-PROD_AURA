...
// =====================================================
// PRESETS D'ÉCHÉANCIERS DE PAIEMENT (payment_schedule_presets)
// =====================================================

export async function listPaymentSchedulePresets(companyId: string): Promise<PaymentSchedulePreset[]> {
  const { data, error } = await supabase
    .from('payment_schedule_presets')
    .select('*')
    .eq('company_id', companyId)
    .order('name', { ascending: true });
...
export async function getPaymentSchedulePreset(id: string): Promise<PaymentSchedulePreset> {
  const { data, error } = await supabase
    .from('payment_schedule_presets')
    .select('*')
    .eq('id', id)
...
export async function createPaymentSchedulePreset(
  companyId: string,
  payload: Omit<PaymentSchedulePreset, 'id' | 'company_id' | 'created_at' | 'updated_at'>
): Promise<PaymentSchedulePreset> {
  const { data, error } = await supabase
    .from('payment_schedule_presets')
    .insert({
      company_id: companyId,
...
export async function updatePaymentSchedulePreset(
  id: string,
  payload: Partial<Omit<PaymentSchedulePreset, 'id' | 'company_id' | 'created_at' | 'updated_at'>>
): Promise<PaymentSchedulePreset> {
  const { data, error } = await supabase
    .from('payment_schedule_presets')
    .update(payload)
...
export async function deletePaymentSchedulePreset(id: string): Promise<void> {
  const { error } = await supabase
    .from('payment_schedule_presets')
    .delete()
    .eq('id', id);
